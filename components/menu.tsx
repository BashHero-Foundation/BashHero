  ...
  return (
    <div ref={sidebarRef} className={`fixed left-0 z-40 w-80 transition-transform duration-300 ease-in-out ${open ? "translate-x-0" : "-translate-x-full"}`} style={{ top: "calc(var(--header-height) + 4px)",height: "calc(100vh - 5rem)" }}>
      <button onClick={() => setOpen((s) => !s)}
        className="absolute -right-13 top-5 bg-btn-primary-bg text-white px-3 py-2 rounded-r-md shadow-lg hover:opacity-90 transition"
        aria-expanded={open}
    ...
    <aside className="w-80 h-full bg-bg-body border-r border-border-separator  shadow-2xl overflow-auto">
    <div className="px-4 py-3 border-b border-border-separator flex items-center justify-between">
      ...
      <div className="mt-3 flex flex-col gap-6 rounded-xl p-3">
